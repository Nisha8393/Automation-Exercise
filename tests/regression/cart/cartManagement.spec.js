...
test.describe("Cart Management", { tag: "@regression" }, () => {
  ...
  test(
    "Add product with quantity > 1 - Cart shows correct quantity",
    { tag: "@R-CART-09" },
    async ({
      header,
      productsPage,
  ...
  test(
    "Verify total price calculation - Total equals price x quantity",
    { tag: "@R-CART-10" },
    async ({
      header,
      productsPage,
  ...
  test(
    "Verify cart subtotal - Each product total is correct",
    { tag: "@R-CART-11" },
    async ({ home, viewCartPage, isolatedPage }) => {
  ...
  test(
    "Proceed to checkout without login - Shows Register/Login modal",
    { tag: ["@R-CART-15", "@S-CP-03"] },
    async ({ home, viewCartPage }) => {
      await home.addProductToCart(PRODUCT_A);
      await expect(home.cartModal).toBeVisible();
      await home.clickCartModalViewCart();
  ...
  test(
    "Remove single product - Product removed from cart",
    { tag: "@R-CART-12" },
    async ({ home, viewCartPage, isolatedPage }) => {
      // Add a product
      await home.addProductToCart(PRODUCT_A);
  ...
  test(
    "Remove all products - Cart is empty",
    { tag: "@R-CART-13" },
    async ({ home, viewCartPage, isolatedPage }) => {
      // Add two products
      await home.addProductToCart(PRODUCT_A);
  ...
  test(
    "Verify empty cart state - Shows empty message and link",
    { tag: "@R-CART-14" },
    async ({ home, viewCartPage, isolatedPage }) => {
