...
test.describe("Registration Tests", { tag: "@regression" }, () => {
  ...
  test.describe("Valid Registration", () => {
    ...
    test(
      "should proceed to registration form with valid data",
      { tag: ["@R-AUTH-17", "@S-AP-02"] },
  ...
  test.describe("Invalid Registration", () => {
    test(
      "Register with empty name on signup - Shows 'Please fill out this field.' validation",
      { tag: "@R-AUTH-03" },
      async ({ loginPage, isolatedPage }) => {
    ...
    test(
      "Register with invalid email on signup - Shows browser validation on email",
      { tag: "@R-AUTH-04" },
      async ({ loginPage, isolatedPage }) => {
    ...
    test(
      "Register with empty required fields on registration form - All required fields show validation",
      { tag: "@R-AUTH-05" },
      async ({ loginPage, registerPage, isolatedPage }) => {
        const newUser = signupTestData.generateNewUser();
    ...
    test(
      ...
      { tag: "@R-AUTH-02" },
      async ({ loginPage }) => {
        const testData = signupTestData.invalid.find(
          (t) => t.id === "SIGNUP-01",
        );
